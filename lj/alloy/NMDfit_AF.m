...
% PT_PERC=0.0005; INV_PERC = 0.5;

%0.05
%8x
str.NMD = '/home/jason/lammps/LJ/alloy/10K/0.05/10x/NMD_AF/1';
PT_PERC=0.1; INV_PERC = 0.25; gamma_guess = 4; NUM_WEIGHT = 1;
...
if imode<150
    ...
    PT_PERC=0.002; INV_PERC = 0.25; gamma_guess = 4;
end
...
loglog(SED.sedfreq(:),SED.life(:),'.',...
    SED.sedfreq(:),1E5*SED.sedfreq(:).^(-4))

pause

plot(SED.HLDfreq,SED.sedfreq,'.',...
    SED.HLDfreq,SED.HLDfreq)

pause

plot(SED.sedfreq,SED.R2(:),'.')

...
str.main = strcat('/home/jason/lammps/LJ/alloy/10K/0.0/8x/NMD/');
...
%read data
     ...
     dummy = dlmread(strcat(str.main,'x0_0_1.data'));
    
%     dummy = dlmread(strcat(str.main,'x0_0.05_1.data'));
    
%     dummy = dlmread(strcat(str.main,'x0_0.15_1.data'));
...
%replace double alloy
    I = find(LD.m ==1); LD.m(I) = LD.mass(1);
    I = find(LD.m ==2); LD.m(I) = LD.mass(2);
...
%calculate the phi1 and phi2    
    ...
    LD.phi1 = -48*LD.r14i + 24*LD.r4.*LD.r2.*LD.r14i;    
    LD.phi2 = (4*156)*LD.r14i - (42*4)*LD.r4.*LD.r2.*LD.r14i;
...
%Lorentzian
LD.dw_avg = 5.0*real(mean(LD.freq(2:LD.NUM_MODES)-LD.freq(1:LD.NUM_MODES-1)));
LD.delwij = ...

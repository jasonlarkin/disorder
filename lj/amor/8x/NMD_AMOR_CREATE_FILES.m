...
NMD.walltime.matlab = 24; NMD.cpu.matlab = 1; NMD.mem.matlab = 6;
...
dummy =...
    dlmread(...
    strcat(NMD.str.main,'/prepare/LJ_amor_',int2str(NMD.seed.amor),'.pos'));
    ...
    NMD.mass(1:size(NMD.x0,1)) = NMD.m(1);
    
    NMD.NUM_ATOMS_UCELL = NMD.param(2);
    NMD.NUM_MODES = NMD.param(2)*3; NMD.NUM_ATOMS = size(NMD.x0,1);
    NMD.NUM_ATOMS_UCELL = NMD.param(2);
    ...
    NMD.freq =...
    dlmread(...
    strcat(NMD.str.main,'/prepare/freq_',int2str(NMD.seed.amor),'.dat')); 
...
%for NMD.seed.amor=1:size(NMD.seed.amor,2)
    str.cmd = ['mkdir -p ./' int2str(NMD.seed.amor) '/NMD'];
    system(str.cmd);

    str.cmd = ['mkdir -p ./' int2str(NMD.seed.amor)];
    system(str.cmd);
    
    str.cmd = ['cp ./lmp_submit.sh.temp ./' int2str(NMD.seed.amor) '/lmp_submit.sh'];
    system(str.cmd);    
%output lammps    
    str.orig = 'NUM_ATOMS';
    ...
    str.orig = 'LY';
    str.change = [num2str( NMD.Ly )];
    str.cmd4 = ['-e ''s/\<' str.orig '\>/' str.change '/g'' '];
    str.orig = 'LZ';
    str.change = [num2str( NMD.Lz )];
    ...
    str.cmd8 =...
        ['lmp.in.x0.temp > ./' int2str(NMD.seed.amor) '/lmp.in.x0.' int2str(NMD.seed.amor)];
...
str.write=...
    strcat(...
    NMD.str.main,['/' int2str(NMD.seed.amor) '/lmp.in.x0.' int2str(NMD.seed.amor)]);
...
%lmp_ISEED.sh------------------------------------------------------        
        ...
        str.cmd1 = ['-e ''s/\<' str.orig '\>/' str.change '/g'' '];
        str.orig = 'runpath';
        str.change = strcat(NMD.str.main,'/',int2str(NMD.seed.amor));
    ...
    str.cmd5 = ['lmp.sh.temp > ./' int2str(NMD.seed.amor) '/lmp' int2str(iseed) '.sh'];
...
%lmp.in.sed.iseed
        str.orig = 'IN.X0';
        str.change = ['lmp.in.x0.' int2str(NMD.seed.amor)];
        str.cmd1 = ['-e ''s/\<' str.orig '\>/' str.change '/g'' '];
        str.orig = 'LMP_TEMP';
        str.change = ['lmp.in.sed.' int2str(iseed)];
        str.cmd2 = ['-e ''s/\<' str.orig '\>/' str.change '/g'' '];
        str.orig = 'IX0';
        str.change = [int2str(NMD.seed.amor)];
        str.cmd3 = ['-e ''s/\' str.orig '>/' str.change '/g'' '];
        str.orig = 'ISEED_TMP';
        ...
        str.cmd9 =...
            ['lmp.in.sed.temp > ./' int2str(NMD.seed.amor) '/lmp.in.sed.' int2str(iseed)];
    ...
    str.write = strcat(NMD.str.main,'/',int2str(NMD.seed.amor),'/lmp_submit.sh');
    dlmwrite(str.write,output,'-append','delimiter','');
...
system(...
    strcat('cp ./NMD_submit.sh.temp ./',int2str(NMD.seed.amor),'/NMD_submit.sh'));
...
%NMD_ISEED_IKSLICE.sh------------------------------------------------------        
        ...
        str.cmd3 = ['-e ''s/\<' str.orig '\>/' str.change '/g'' '];
    str.cmd4 =...
        ['NMD.sh.temp > ./' int2str(NMD.seed.amor) '/NMD_' int2str(iseed) '_' int2str(imode) '.sh'];
...
%NMD_ISEED_IKSLICE.m-------------------------------------------------------        
        ...
        str.cmd2 = ['-e ''s/\<' str.orig '\>/' str.change '/g'' '];
        str.cmd3 =...
            ['NMD.m.temp > ./' int2str(NMD.seed.amor) '/NMD_' int2str(iseed) '_' int2str(imode) '.m'];
...
%NMD_submit.sh------------------------------------------------------------- 
    ...
    str.write = strcat(NMD.str.main,'/',int2str(NMD.seed.amor),'/NMD_submit.sh');
    dlmwrite(str.write,output,'-append','delimiter','');
...
%NMD_grep.m-------------------------------------------------------        
    str.cmd3 = ['NMD.m.temp > ./' int2str(NMD.seed.amor) '/NMD_' int2str(iseed) '_' int2str(imode) '.m'];
    str.cmd = ['sed ' str.cmd1 str.cmd2 str.cmd3];
    system(str.cmd);


system(strcat('cp ./NMD_grep.m.temp ./',int2str(NMD.seed.amor),'/NMD_grep.m'));

%SAVE NMD structure--------------------------------------------------------  

save(strcat(NMD.str.main,'/',int2str(NMD.seed.amor),'/NMD.mat'), '-struct', 'NMD');

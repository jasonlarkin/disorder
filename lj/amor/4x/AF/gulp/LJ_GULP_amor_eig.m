...
for iatom = 1:size(x,1)-1      
    ...
    str.change = ['Ar core ' num2str(x(iatom+1,3:5)*LJ.sigma*1E10) ' 0 1 1 1'];
...
%replace LX,Y,Z tags 
        str.orig = strcat('LX');
    str.change = num2str(x(1,3)*LJ.sigma*1E10);
    str.cmd = strcat('sed ''s/',str.orig,'/',str.change,...
        '/g'' amor_eig_template3.gin > amor_eig_template4.gin');
    system(str.cmd);
    
        str.orig = strcat('LY');
    str.change = num2str(x(1,4)*LJ.sigma*1E10);
    str.cmd = strcat('sed ''s/',str.orig,'/',str.change,...
        '/g'' amor_eig_template4.gin > amor_eig_template5.gin');
    system(str.cmd);
    
        str.orig = strcat('LZ');
    str.change = num2str(x(1,5)*LJ.sigma*1E10);
    str.cmd = strcat('sed ''s/',str.orig,'/',str.change,...

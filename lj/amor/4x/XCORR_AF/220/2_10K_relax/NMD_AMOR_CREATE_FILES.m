...
NMD.walltime.lammps = 2; NMD.cpu.lammps = 1; 
...
%replace double alloy
    I = find(NMD.mass(:,1) ==1); NMD.mass(I) = NMD.m(1);
    I = find(NMD.mass(:,1) ==2); NMD.mass(I) = NMD.m(2);
...
%NMD_ISEED_IKSLICE.m-------------------------------------------------------        
        ...
        str.cmd2 = ['-e ''s/\<' str.orig '\>/' str.change '/g'' '];
        str.cmd3 =...
            ['NMD.pos.m.temp > ./' int2str(NMD.seed.alloy) '/work/NMD_' int2str(imode) '.m'];

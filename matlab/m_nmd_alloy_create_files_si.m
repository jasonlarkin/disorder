...
clear
%--------------------------------------------------------------------------
    nmd.str.main = '/home/jason/disorder2/si/alloy/0.5/4x/' ;
...
change(6).str = num2str(nmd.x0.mass(1));

m_change_file_strings(...
    [nmd.str.matlab 'lmp.in.x0.alloy.single.tmp'],...
    orig,...
...
%lmp.in.sed.iseed
    orig(1).str = 'IN.X0';
    change(1).str = ['lmp.in.x0.' int2str(nmd.x0.alloy_seed)];
    orig(2).str = 'LMP_TMP';
...
system(...
    ...
    nmd.str.matlab 'nmd.submit.sh']);

for iseed=1:size(nmd.seed.initial,2)
    for ikslice = 1:nmd.NUM_KSLICES
%nmd_ISEED_IKSLICE.sh------------------------------------------------------        
    orig(1).str = 'runpath';
    change(1).str = strcat(nmd.str.main);
...
%nmd_ISEED_IKSLICE.m------------------------------------------------------- 
    ...
    m_change_file_strings(...
    [nmd.str.matlab 'm_nmd_sed.m.tmp'],...
    orig,...

...
%KPTLIST: Load kpt list
    str_read=strcat(str_main,'NMD\conv\kptlist.dat');
    %Linux
...
%INITIAL POSITIONS: Set initial positions for id matching
    %Windoze
    str_read=strcat(str_main,'NMD\conv\x0.data');
    %Linux
...
%SED PARAMETERS: load SED parameters
    %Windoze
    str_read=strcat(str_main,'NMD\conv\SED_param.dat');
    %Linux
...
%EIGENVECTORS
        %Windoze
            str_read=strcat(str_main,'NMD\conv\eigvec.dat');
        %Linux
            %str_read=strcat(str_main,'NMD/1atom/eigvec.dat');
        eigenvec = dlmread(str_read);
%FREQUENCIES
        %Windoze
        str_read=strcat(str_main,'NMD\conv\freq.dat');
        %Linux
        %str_read=strcat(str_main,'NMD/1atom/freq.dat');
        freq = load(str_read);
%GROUP VEL
        %Windoze
        str_read=strcat(str_main,'NMD\conv\vel.dat');
        %Linux
        %str_read=strcat(str_main,'NMD/1atom/freq.dat');
        group_vel = load(str_read);
            
%READ IN FIRST FILE        
        str_read=strcat(str_main,'NMD\conv\SED_single_1.txt');
    ...
    %------AVG OVER SEEDS------------------------------------------------------
        for i1=1:NUM_SEED
                %ME
                str_read=strcat(str_main,'NMD\conv\SED_single_',int2str(i1),'.txt');
                %ALEX

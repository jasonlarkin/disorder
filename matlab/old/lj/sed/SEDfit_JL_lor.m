...
%Loop over grouped peaks and fit 
        ...
        for group_id = 1:group_loc(length(group_loc))               
            ...
            for i3=1:length(Igroup)
                if length(Igroup)==1
                strtemp= strcat('(0.1592)*( c(',int2str(i3),'))./((w - c(',int2str(i3+1),')).^2 + (c(',int2str(i3),')/2).^2)');
                str_func = strcat(str_func,strtemp);
                else
                strtemp= strcat(' + (0.1592)*(c(',int2str(2*(i3-1)+1),'))./((w - c(',int2str(2*(i3-1)+2),')).^2 + (c(',int2str(2*(i3-1)+1),')/2).^2)');
                str_func = strcat(str_func,strtemp);
                end  
            end
...
%                     PT_PERC*SED.irrkpt.sedavg(loc(Igroup(1)),kpt_list(kpt_cnt))
            ...
            %Build intial guess array
                    for i3=1:length(Igroup)   
                        if length(Igroup)==1
                        c0(1+(i3-1)*2:i3*2) = [ gamma_guess, w(loc(Igroup(i3))) ];
                        else
            ...
            hold on
                    Igroup = find(group_loc==group_id);
                        for i3=1:length(Igroup)  
                            ...
                            %end
                            center=c_fit(2*i3)*w_step;
                            lifetime=1/(2*c_fit(2*i3-1))/w_step;
                            %Store in rads/ps

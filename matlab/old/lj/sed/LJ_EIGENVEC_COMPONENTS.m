...
%EIGENVECTORS
        %Windoze
            str_read=strcat(str_main,'NMD\4atom\eigvec.dat');
        %Linux
            %str_read=strcat(str_main,'NMD/1atom/eigvec.dat');
        eigenvec = dlmread(str_read);
%FREQUENCIES
        %Windoze
        str_read=strcat(str_main,'NMD\4atom\freq.dat');
        %Linux
        ...
        for imode = 1:NUM_MODES
            for ikpt = 1:NUM_KPTS
                eigx = eigenvec(((NUM_ATOMS_UCELL*3)*(ikpt-1)+1):3:((NUM_ATOMS_UCELL*3)*ikpt),imode);
                eigy = eigenvec(((NUM_ATOMS_UCELL*3)*(ikpt-1)+2):3:((NUM_ATOMS_UCELL*3)*ikpt),imode);
                eigz = eigenvec(((NUM_ATOMS_UCELL*3)*(ikpt-1)+3):3:((NUM_ATOMS_UCELL*3)*ikpt),imode); 
                sum( eigx.*conj(eigx) + eigy.*conj(eigy) + eigz.*conj(eigz) );

                ratio(ikpt,imode,1) = sum(real(eigx).^2 + real(eigy).^2 + real(eigz).^2) / sum(real(eigx).^2 + imag(eigx).^2 + real(eigy).^2 + imag(eigy).^2 + real(eigz).^2 + imag(eigz).^2) ;
                
                %plot(real(eigx),imag(eigx),'.',real(eigy),imag(eigy),'.',real(eigz),imag(eigz),'.')     
            end
        end
        
        for imode=1:NUM_MODES
        hold on
            plot(1:NUM_KPTS,ratio(:,imode,1),'.')
        end

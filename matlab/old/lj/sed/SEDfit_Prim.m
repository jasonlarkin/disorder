...
%Set main directory and read in first file
        str_main=strcat('E:\CMU\work\Phonons\LJArgon\Solid\crystal\perfect\SED\8x\1K\1atom\');
        str=strcat(str_main,'SED_1.txt');
...
%SED PARAMETERS: load SED parameters
    ...
    %------AVG OVER SEEDS------------------------------------------------------
        for i1=1:NUM_SEEDS
                %ME
                str=strcat(str_main,'SED_',int2str(i1),'.txt');
                %ALEX
                ...
                %SED.kpt(1:NUM_KPTS,1:3) => [kx ky kz];
                    for i2=1:NUM_KPTS
                    SED.redkpt.sed(:,i2) = SED.redkpt.sed(:,i2) + dummy((i2-1)*(SAMPLE_RATE+1)+2:(i2)*(SAMPLE_RATE+1),2);
    ...
    %------FIND NUMBER IRR KPTS AND AVG------------------------------------ 
        ...
        SED.irrkpt.sedavg(1:SAMPLE_RATE,SED.irrkpt.numirr)=0;
...
%                     if kz==0.5; kz = kz-KPT_TOL; end
        ...
        semilogy(1:w_plot,SED.irrkpt.sedavg(1:w_plot,kpt_list(kpt_cnt)),'.')
...
%                     PT_PERC*SED.irrkpt.sedavg(loc(Igroup(1)),kpt_list(kpt_cnt))
            ...
            %Build intial guess array
                    for i3=1:length(Igroup)   
                        if length(Igroup)==1
                        c0(1+(i3-1)*3:i3*3) = [ SED.irrkpt.sedavg(loc(Igroup(i3)),kpt_list(kpt_cnt)), 0.5*gamma_guess, w(loc(Igroup(i3))) ];
                        else
                        ...
                        %multiple lorentzians have 1/N (1/3 for N=3 example) the width
                            ...
                            c0(1+(i3-1)*3:i3*3) = [ SED.irrkpt.sedavg(loc(Igroup(i3)),kpt_list(kpt_cnt)), 0.5*gamma_guess, w(loc(Igroup(i3))) ];    
                                ...
                                %gamma_guess = 2;
                            c0(1+(i3-1)*3:i3*3) = [ SED.irrkpt.sedavg(loc(Igroup(i3)),kpt_list(kpt_cnt)), 0.5*gamma_guess/length(Igroup), w(loc(Igroup(i3))) ];    
                            end
                        end
                    end 
            ...
            lb(1:length(c0)) = 0.0; ub(1:length(c0)) = max(SED.irrkpt.sedavg(:,kpt_list(kpt_cnt)))*10000;
            ...
            %Plot each fit, single and multiple
                        ...
                        axis([0 w_plot 0.0001*max(SED.irrkpt.sedavg(:,kpt_list(kpt_cnt))) max(SED.irrkpt.sedavg(:,kpt_list(kpt_cnt)))]);
        end

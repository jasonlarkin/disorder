...
%1) read_data
        ...
        %str_main='E:\CMU\work\Phonons\Lattice_Dynamics\Matlab\';
        %str=strcat(str_main,'LJ_rand_mass_gauss.in.data');
...
%2) Build up the unit cell images        
        ...
        %    x_ucell =
        %    dlmread('D:\CMU\work\Phonons\Lattice_Dynamics\Matlab\Ar_reduce
        %    d_units_matlab.xyz');       %read in initial positions    
...
%4) Calculate dynamical matrix 
       ...
       k=-(2*pi/alat(1,1)):dk:(2*pi/alat(1,1));
        ...
        for j=1:length(k)
           kappa = [k(j),0,0];
           i=sqrt(-1);
           [D,D2] = dynam_matrix_4(a2,x,x_ucell,m,kappa);
           freq(j,1:length(eig(D)))=abs(sqrt(eig(D))); 
        end
        
        plot(freq)
        
        %[D] = dynam_matrix(a2,x,x_ucell,m,[0.0,0.0,0.0]);
        %[D] = dynam_matrix_2(a2,x,x_ucell,m,[0.0,0.0,0.0]);
        %[D] = dynam_matrix_3(a2,x,x_ucell,m,[0.0,0.0,0.0]);
        
        %GAMMA POINT calculation
%         [D,PHI1,PHI2,PHI3] = dynam_matrix_4(a2,x,x_ucell,m,[0.0,0.0,0.0]);
...
%8) Calculate thermal diffusivity matrix
        ...
        %[Diff,S] = therm_diff_5(D,eigV,W,V,hbar,epsilon_Ar,tau_Ar,a2,x,x_ucell,m,[0.0,0.0,0.0]);

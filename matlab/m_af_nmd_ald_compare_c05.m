...
loglog(...
    ...
    linspace(...
    min(vcnmd(ipt).sed.freq/sqrt(VIRTUAL_MASS)),...
    max(vcnmd(ipt).sed.freq/sqrt(VIRTUAL_MASS)),100),...
    ones(100,1)*...
    vcnmd(1).nmd.alat/2*lj.sound*(1/3),...
    linspace(...
    min(vcnmd(ipt).sed.freq/sqrt(VIRTUAL_MASS)),...
    max(vcnmd(ipt).sed.freq/sqrt(VIRTUAL_MASS)),100),...
    (1/3)*(lj.sound)^2.*2*pi./linspace(...
    min(vcnmd(ipt).sed.freq/sqrt(VIRTUAL_MASS)),...
    max(vcnmd(ipt).sed.freq/sqrt(VIRTUAL_MASS)),100)...
    )
subplot(3,1,3),...

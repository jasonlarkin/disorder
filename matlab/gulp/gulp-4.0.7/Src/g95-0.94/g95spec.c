...
static void lookup_option(Option *xopt, int *xskip, const char **xarg,
			  const char *text) {
Option opt = OPTION_;
int skip;
const char *arg = NULL;
    ...
    else if (strcmp(text, "cpp") == 0)
	opt = OPTION_cpp, skip = 0;
	...
	else if (!strcmp(text, "-nostdlib") || !strcmp(text, "-nodefaultlibs"))
	    opt = OPTION_nostdlib;
...
static void append_arg(const char *arg) {
static int newargsize;
    ...
    if (g77_newargv == g77_xargv && g77_newargc < g77_xargc &&
    ...
    if (g77_newargv == g77_xargv) { /* Make new arglist. */
	int i;
...
", version_string);
	...
	case OPTION_help:
	    /* Let gcc.c handle this, as it has a really cool facility
	     * for handling --help and --verbose --help.  */
	    return;
...
/* Modified for g95 by Dave Miles to support MinGW 'gethostname' */
...
int lang_specific_pre_link(void) { /* Not used for F95. */
...
int lang_specific_extra_outfiles = 0;  /* Not used for F75. */

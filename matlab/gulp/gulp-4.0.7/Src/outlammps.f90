...
!
!  Julian Gale, NRI, Curtin University, November 2012
!
  use current
...
!
      if (npottypes(2).gt.0) then
        write(itmp,'(/,''pair_style overlay coul/long '',f6.3,'' lj/smooth/AB '',2(f6.3,2x))') cutp,tapermin,cutp
      elseif (npottypes(12).gt.0.or.npottypes(13).gt.0) then
        write(itmp,'(/,''pair_style overlay coul/long '',f6.3,'' lj/smooth/gulp '',2(f6.3,2x))') cutp,tapermin,cutp
      elseif (npottypes(1).gt.0) then
        write(itmp,'(/,''pair_style overlay coul/long '',f6.3,'' buck/smooth/gulp '',2(f6.3,2x))') cutp,tapermin,cutp
      endif
    else
...
!
          if ((cutp-tapermin).gt.1.0d-12) then
            write(line(iline+1:iline+18),'('' buck/smooth/gulp '')')
            iline = iline + 18
...
!
          if ((cutp-tapermin).gt.1.0d-12) then
            write(line(iline+1:iline+14),'('' lj/smooth/AB '')')
            iline = iline + 14
            write(line(iline+1:iline+13),'(f6.3,1x,f6.3)') tapermin,cutp
            iline = iline + 13
          else
            write(line(iline+1:iline+6),'('' lj/AB '')')
            iline = iline + 6
            write(line(iline+1:iline+6),'(f6.3)') cutp
            iline = iline + 6
          endif
...
!
          if ((cutp-tapermin).gt.1.0d-12) then
            write(line(iline+1:iline+16),'('' lj/smooth/gulp '')')
            iline = iline + 16
            write(line(iline+1:iline+13),'(f6.3,1x,f6.3)') tapermin,cutp
            iline = iline + 13
          else
...
!
    ...
    do isp1 = 1,nspec
      do isp2 = 1,isp1
        ind = ind + 1
        if (npotspecpair(ind).eq.0) then
!
!  No potentials so just write out Coulomb
!
          write(itmp,'(''pair_coeff '',i3,1x,i3,'' coul/long '')') isp1,isp2
        else
!
...
!
               ...
               if (npottype.eq.1) then
                 write(itmp,'(''pair_coeff '',i3,1x,i3,1x,f12.10,1x,f13.10,1x,f8.4,1x,f8.4)') &
                   isp1,isp2,eps,sig,rpot2(npotokptr(n)),rpot(npotokptr(n))
               else
                 if ((cutp-tapermin).gt.1.0d-12) then
                   write(itmp,'(''pair_coeff '',i3,1x,i3,'' lj/smooth/gulp '',f12.10,1x,f13.10,1x,f8.4,1x,f8.4)') &
                     isp1,isp2,eps,sig,rpot2(npotokptr(n)),rpot(npotokptr(n))
                 else
...
!
               if (twopot(2,n).gt.1.0d-12.and.tpot(1,n).eq.12.0_dp.and.tpot(2,n).eq.6.0_dp) then
                 sig = (2.0_dp*twopot(1,n)/twopot(2,n))**sixth
                 eps = twopot(1,n)/sig**12.0_dp
                 if (npottype.eq.1) then
                   write(itmp,'(''pair_coeff '',i3,1x,i3,1x,f12.10,1x,f13.10,1x,f8.4,1x,f8.4)') &
                     isp1,isp2,eps,sig,rpot2(npotokptr(n)),rpot(npotokptr(n))
                 else
                   if ((cutp-tapermin).gt.1.0d-12) then
                     write(itmp,'(''pair_coeff '',i3,1x,i3,'' lj/smooth/AB '',f12.10,1x,f13.10,1x,f8.4,1x,f8.4)') &
                       isp1,isp2,eps,sig,rpot2(npotokptr(n)),rpot(npotokptr(n))
                   else
                     write(itmp,'(''pair_coeff '',i3,1x,i3,'' lj/AB '',f12.10,1x,f13.10,1x,f8.4,1x,f8.4)') &
                       isp1,isp2,eps,sig,rpot2(npotokptr(n)),rpot(npotokptr(n))
                   endif
                 endif
               endif
...
!
               if (npottype.eq.1) then
                 write(itmp,'(''pair_coeff '',i3,1x,i3,1x,f12.10,1x,f13.10,1x,f8.4,1x,f8.4)') &
                   isp1,isp2,twopot(1,n),twopot(2,n),twopot(3,n),rpot2(npotokptr(n)),rpot(npotokptr(n))
               else
                 if ((cutp-tapermin).gt.1.0d-12) then
                   write(itmp,'(''pair_coeff '',i3,1x,i3,'' buck/smooth/gulp '',f12.10,1x,f13.10,1x,f8.4,1x,f8.4)') &
                     isp1,isp2,twopot(1,n),twopot(2,n),twopot(3,n),rpot2(npotokptr(n)),rpot(npotokptr(n))
                 else
  ...
  write(itmp,'(/,''special_bonds lj 0.0 0.0 1.0 coul 0.0 0.0 1.0'',/)')
!

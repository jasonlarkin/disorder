...
!
      ...
      elseif (nthrty(i).eq.22) then
        if (mmtexc(i).eq.1) then
          ...
          if (lfit) then
            write(iout,'(1x,f8.6,1x,3i3,1x,4i2)') the,(nint(threepoly(k,i)),k=1,3),(itmp(5*(i-1)+j),j=1,4)
          else
            write(iout,'(1x,f8.6,1x,3i3)') the,(nint(threepoly(k,i)),k=1,3)
          endif
        else

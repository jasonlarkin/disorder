...
!  11/12 Unused argument removed from call to functsync
!  11/12 Unused argument from call to lmbfgssub removed
!
!  Conditions of use:
!
...
!
    ...
    call endstring(arcfilel,len(arcfilel),indend)
    if (indend.eq.0) then
...
!
  ...
  nhigher = 2
  iflag = 1
  call functsync(iflag,niter,nrep,nrepptr,nvar,xcrep,fcrep,gcrep,tangent,fctot,nlower)
  nlower = 2
  nhigher = 1
  iflag = 1
  call functsync(iflag,niter,nrep,nrepptr,nvar,xcrep,fcrep,gcrep,tangent,fctot,nlower)
...
!
      call lmbfgssub(niterlocal,nvar,5_i4,stp,gcrep(1,nlower),diag,hessian,maxhess,pcrep)
!

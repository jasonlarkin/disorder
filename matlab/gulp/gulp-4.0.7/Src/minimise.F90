...
!
  ...
  use interupt,    only : controlC, sigint
  use iochannels
  use optimisation
  use parallel
...
!
  ...
  real(dp)                                    :: yhy
!
  external                                    :: trap_sigint
!
!  Call sig handler
!
  call csignal(sigint,trap_sigint)
!
...
!************************
    ...
    call lmbfgssub(jnrst,nvari,lmbfgsorder,alp,gc,hdiag,hessian,maxhess,pvect)
...
!
        ...
        enddo
!
!  Initial set to steepest descents since this is more robust
!
        do i = nmin,nvari
          pvect(i) = gc(i)
        enddo
...
!  Check time
!
  if (iflag.ge.0.and.jcyc.lt.maxcal.and.(.not.controlC.or.lrelax)) goto 10
  if (iflag.lt.0) then
    ifail = - 1
  else
    ifail = 2

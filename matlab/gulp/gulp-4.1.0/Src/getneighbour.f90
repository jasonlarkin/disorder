...
!  11/12 Initialisation of arrays changed from number to nREBOatom to avoid out of
!        bounds error for mixed potential models
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!
  if (nprocs.gt.1) then
    neighno(1:maxneigh,1:nREBOatom) = 0
    nREBObond(1:maxneigh,1:nREBOatom) = 0
    rneigh(1:maxneigh,1:nREBOatom) = 0.0_dp
    xneigh(1:maxneigh,1:nREBOatom) = 0.0_dp
    yneigh(1:maxneigh,1:nREBOatom) = 0.0_dp
    zneigh(1:maxneigh,1:nREBOatom) = 0.0_dp
  endif
  nneigh(1:nREBOatom) = 0
!****************************************
!  Calculate neighbour lists for atoms  *
!****************************************
  if (lspatialok) then

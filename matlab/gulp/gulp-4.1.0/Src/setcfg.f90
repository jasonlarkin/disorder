...
!
        ...
        if (ntdfieldcfg(ii).gt.0) then
          write(ioout,'(/,''  Number of TD-Electric fields = '',i13)') ntdfieldcfg(ii)
          do j = 1,ntdfieldcfg(ii)
            write(ioout,'(/,''  TD-Electric field applied    = '',f13.6,'' eV/Ang.e '')') td_fieldcfg(1,j,ii)
            write(ioout,'(''  Time constant (rate)  B      = '',f13.6,'' ps^-1 '')') td_fieldcfg(2,j,ii)
            write(ioout,'(''  Time constant (phase) C      = '',f13.6)') td_fieldcfg(3,j,ii)
            write(ioout,'(/,''  TD-Electric field direction  = '',f13.6,'' a '')') td_fielddirectioncfg(1,j,ii)
            write(ioout,'(''                                 '',f13.6,'' b '')') td_fielddirectioncfg(2,j,ii)
            write(ioout,'(''                                 '',f13.6,'' c '')') td_fielddirectioncfg(3,j,ii)
          enddo
        endif
      elseif (ndimen(ii).eq.2) then
        if (.not.lterseincell) then
...
!
    if (ioproc) then
      call rlist
      if (nthb.gt.0.and.langle) call angle(ioout,0_i4,nangtot)
      if (nfor.gt.0.and.ltors) call torsion(ioout,0_i4,nphitot)
    endif
!*********************************

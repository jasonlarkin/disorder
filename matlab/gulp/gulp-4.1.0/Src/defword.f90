...
!  ndintptr= pointer to interstitials for region 2a
!  ndvacptr= pointer to vacancies for region 2a
!  nvaca   = no. of vacancies
!  ninte   = no. of interstitials
!  nreldef = perfect lattice atom to which defect is equivalent
!  ldeflin = .true. if list of vacancies and interstitials is input
!            for restart using deflist
...
!   3/13 Check on dimensions of maxvacint arrays added
!   3/13 Use of scratch files removed
!   3/13 Meaning of nreldef value changed
!   3/13 dlist_vacancy/interstitial options added to replace deflist
!
!  Conditions of use:
!
...
!
  ...
  if (index(word,'dlist_i').eq.1) goto 200
  if (index(word,'dlist_v').eq.1) goto 210
  return
!******************
!  Defect centre  *
!******************
...
128 continue
!
!  Check memory for defect information over all configurations
!
...
!
!  Copy defect information to configuration arrays
!
  nreg1cfg(ncurr) = nreg1
  do i = 1,nreg1
    nreg1tot = nreg1tot + 1
    natdefecfg(nreg1tot) = nat(i)
    ntypdefecfg(nreg1tot) = nftype(i)
    xdefecfg(nreg1tot) = xdefe(i)
    ydefecfg(nreg1tot) = ydefe(i)
    zdefecfg(nreg1tot) = zdefe(i)
    qdefecfg(nreg1tot) = qdefe(i)
    occdefecfg(nreg1tot) = occdefe(i)
    radefecfg(nreg1tot) = radefe(i)
    ndefmolcfg(nreg1tot) = natmol(i)
    ndefindcfg(nreg1tot) = nmolind(i)
    ldefbsmatcfg(nreg1tot) = ldefbsmat(i)
    ldqmatomcfg(nreg1tot) = ldqmatom(i)
    idoptcfg(1,nreg1tot) = itmp(3*(i-1)+1)
    idoptcfg(2,nreg1tot) = itmp(3*(i-1)+2)
    idoptcfg(3,nreg1tot) = itmp(3*(i-1)+3)
  enddo
!
  lwordok = .true.
...
!************
!
!  Check that explicit region 1 has been supplied else deflist is redundant
!
...
!
    ...
    if (ntotvaca+nvaca.gt.maxtotvac) then
      maxtotvac = ntotvaca + nvaca
      call changemaxtotvac
    endif
    if (ntotinte+ninte.gt.maxtotint) then
      maxtotint = ntotinte + ninte
      call changemaxtotint
    endif
!
    allocate(itmp(ntot),stat=status)
    if (status/=0) call outofmemory('defword','itmp')
!
    read(nru,*,err=99) (itmp(i),i=1,ntot)
    do i = 1,nvaca
      ndvacptr(i) = itmp(i)
    enddo
    do i = 1,ninte
      ndintptr(i) = itmp(nvaca+i)
    enddo
    do i = 1,nvaca
      ndvacptrcfg(ntotvaca+i) = ndvacptr(i)
    enddo
    do i = 1,ninte
      ndintptrcfg(ntotinte+i) = ndintptr(i)
    enddo
!
    deallocate(itmp,stat=status)
    if (status/=0) call deallocate_error('defword','itmp')
...
!  Finalise quantities
!
    ntotinte = ntotinte + ninte
    ntotvaca = ntotvaca + nvaca
    nintecfg(ncurr) = ninte
    nvacacfg(ncurr) = nvaca
  endif
  lwordok = .true.
  return
!***********
...
!
!  Save this in configuration arrays
!
    if (nreg1+nreg1tot.gt.maxtotr1at) then
      maxtotr1at = nreg1tot + nreg1
      call changemaxtotr1at
    endif
    ndind = 0
...
!***********************
!
!  Check that explicit region 1 has been supplied else dlist is redundant
!
200 if (ndexplicit.ne.ncurr) then
    nwarn = nwarn + 1
    if (ioproc) then
      call outwarning('dlist_interstitial supplied without region_1 - list ignored',iline)
    endif
  else
    lindintptr(ncurr) = .true.
    if (nfloat.eq.0) then
      line = '  '
      read(nru,'(a)') line
      iline = iline + 1
...
!
    read(nru,*,err=99) (ndintptr(i),i=1,ninte)
    do i = 1,ninte
      ndintptrcfg(ntotinte+i) = ndintptr(i)
    enddo
!
...
!  Check that explicit region 1 has been supplied else dlist is redundant
!
210 if (ndexplicit.ne.ncurr) then
    nwarn = nwarn + 1
    if (ioproc) then
      call outwarning('dlist_vacancy supplied without region_1 - list ignored',iline)
    endif
  else
    lindvacptr(ncurr) = .true.
    if (nfloat.eq.0) then
      line = '  '
      read(nru,'(a)') line
      iline = iline + 1
...
!
    do i = 1,nvaca
      ...
      call linepro(nru,line,iline)
!
!  Check that word is present
!
      if (nword.eq.0) then
        call outerror('Missing atom label in dlist_vacancy',iline)
        call stopnow('defword')
      endif
!
      call ltont(words(1),inat,itype)
      if (nword.ge.2) then
        if ((index(words(2),'s').eq.1).or.(index(words(2),'S').eq.1)) inat = inat + maxele
        if ((index(words(2),'b').eq.1).or.(index(words(2),'B').eq.1)) then
          lbrdvacptrcfg(ntotvaca+i) = .true.
          if ((index(words(2),'s').eq.2).or.(index(words(2),'S').eq.2)) inat = inat + maxele
        else
          lbrdvacptrcfg(ntotvaca+i) = .false.
        endif
      else
        lbrdvacptrcfg(ntotvaca+i) = .false.
      endif
!
      natdvacptrcfg(ntotvaca+i) = inat
      ntypdvacptrcfg(ntotvaca+i) = itype
      xyzdvacptrcfg(1,ntotvaca+i) = floats(1)
      xyzdvacptrcfg(2,ntotvaca+i) = floats(2)
      xyzdvacptrcfg(3,ntotvaca+i) = floats(3)
    enddo
!
!  Finalise quantities
!

...
!   9/12 Check added that existing molecule atom numbers do not 
!        exceed the number of atoms in the structure.
!
!  Conditions of use:
!
...
!
!  Julian Gale, NRI, Curtin University, September  2012
!
  use configurations, only : nascfg
  use constants
    ...
    if (index(words(2),'lowe').eq.1) then
      ...
      if (nword.gt.2) then
        ...
        if (index(mcfile,'.gmc').eq.0) then
          call endstring(mcfile,len(mcfile),iend)
...
!
!  Check whether atom number exceeds the number in the structure
!
      if (nmollist(nmolptr(nm)+j).gt.nascfg(ncurr)) then
        call outerror('atoms in GCMC existing molecules exceed total number of atoms for cfg',iline)
        call stopnow('mcword')
      endif

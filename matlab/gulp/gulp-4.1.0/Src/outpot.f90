...
!
        ...
        lout = .true.
!
!  Bond-angle general cross term 3-body form
!
        do i = 1,nthb
          if (nthrty(i).eq.22.and.iptyp(i).eq.k) then
            if (lout) then
              write(ioout,'(/,''  Bond-Angle General cross form:'',/)')
              write(ioout,'(''--------------------------------------------------------------------------------'')')
              write(ioout,'(''       Atom Types        Powers    Force Cst     R0     Theta      Cutoffs  '')')
              write(ioout,'(''   1       2       3               (eV/rad)    (Angs)   (deg)   1-2   1-3   2-3  '')')
              write(ioout,'(''--------------------------------------------------------------------------------'')')
              lout = .false.
            endif
            ...
            the = theta(i)
            if (mmtexc(i).eq.1) then
              write(ioout,'(a5,1x,a1,1x,a5,1x,a1,1x,a5,1x,a1,3(1x,i2),1x,f10.4,1x,2(f5.3,1x),f6.2,5x,''Bonded'')') &
                lab1,cstyp1,lab2,cstyp2,lab3,cstyp3,(nint(threepoly(l,i)),l=1,3),thbk(i),thrho1(i),thrho2(i),the
            else
              write(ioout,'(a5,1x,a1,1x,a5,1x,a1,1x,a5,1x,a1,3(1x,i2),1x,f10.4,1x,2(f5.3,1x),f6.2,3(1x,f5.2))') &
                lab1,cstyp1,lab2,cstyp2,lab3,cstyp3,(nint(threepoly(l,i)),l=1,3),thbk(i),thrho1(i),thrho2(i),the, &
                thr1min(i),thr2min(i),thr3min(i)
              write(ioout,'(59x,3(1x,f5.2))') thr1(i),thr2(i),thr3(i)
            endif

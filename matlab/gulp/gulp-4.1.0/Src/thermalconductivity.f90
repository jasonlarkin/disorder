...
!
!  Julian Gale, NRI, Curtin University, June 2013
!
...
!
  enddo
!
!  Compute thermal conductivity in W/(m*K)
!
  kappa = 0.0_dp
  if (temperature.lt.1.0d-3) then
  ...
  do i = nfreqmin,mcv
    ...
    kappa = kappa + cv_i*Di(i)
  enddo
!
!  Divide by volume and convert units to SI
!
  vol = volume(rv)
  kappa = 1.0d30*kappa/vol
!
...
!
  if (ioproc) then
    do i = nfreqmin,mcv
      write(ioout,'(i6,2x,f12.4,10x,f24.14)') i,freq(i),Di(i)*1.0d4
    enddo
  endif
!
!  Close output
!
  if (ioproc) then
    write(ioout,'(''--------------------------------------------------------------------------------'')')
    write(ioout,'(''  Thermal conductivity = '',f12.6,'' W/(m.K) '')') kappa
    write(ioout,'(''--------------------------------------------------------------------------------'',/)')
  endif
!

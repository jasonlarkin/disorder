...
!   8/12 Complex part of dielectric constant now output
!
!  On input : 
!
...
!
  ...
  write(ioout,'(''-------------------------------------------------------------------------------'')')
  if (lomegadir) then
    write(ioout,'(''  Omega              Dielectric tensor (real/imaginary)         Reflectivity'')')
  else
    write(ioout,'(''  Omega              Dielectric tensor (real/imaginary)                     '')')
  endif
...
!
    if (lomegadir) then
      write(ioout,'(f8.2,6(1x,f8.4),1x,f10.4)') ome, &
        ...
        reflectivity
      write(ioout,'(8x,6(1x,f8.4))')  &
        aimag(dlcomega(1,1)),aimag(dlcomega(2,1)),aimag(dlcomega(2,2)), &
        aimag(dlcomega(3,1)),aimag(dlcomega(3,2)),aimag(dlcomega(3,3))
    else

...
!
  if (ioproc) then
    ...
    write(ioout,'(''--------------------------------------------------------------------------------'')')
    write(ioout,'('' Mode    : Frequency (cm-1)         Thermal conductivity (10**5 x m**2/s)       '')')
    write(ioout,'(''--------------------------------------------------------------------------------'')')
  endif
!
!  Compute Di values (factors of pi have been cancelled)
!
  if (ioproc) then
    do i = nfreqmin,mcv
      write(ioout,'(i6,2x,f12.4,10x,f24.14)') i,freq(i),Di(i)*10.0d5
    enddo
  endif
!
!  Close output

...
%     (sedald(sys_size_ald(ipt)).vel(Ialloy05,2)/sqrt(1.08)).^2 +...
%     (sedald(sys_size_ald(ipt)).vel(Ialloy05,3)/sqrt(1.08)).^2 ...
%     ),'.',...luckily, we have the folloing available in the house:
%     sedald(sys_size_ald(ipt)).freq(Ialloy05)/sqrt(1.08),...
%     1./( 1./sedald(sys_size_ald(ipt)).life(Ialloy05)/sqrt(1.08) + ...
%     1./(FACTOR_05*alloy_05(sys_size_ald(ipt)).life(Ialloy05))).*...
%     sqrt(...
...
m_si_cond_alloy_ald = m_si_cond_alloy_ald_results;
...
semilogx(...
    ...
    cumtrapz(perfect_05(ipt).mfp_cond)/sum(perfect_05(ipt).mfp_cond)/sqrt(1.08),...
    alloy_05(ipt).mfp_bins*1E9,...
    cumtrapz(alloy_05(ipt).mfp_cond)/m_si_cond_alloy_ald.extrap(2),...
    alloy_5(ipt).mfp_bins*1E9,...
    cumtrapz(alloy_5(ipt).mfp_cond)/m_si_cond_alloy_ald.extrap(4)...
    )

x = alloy_5(ipt).mfp_bins*1E9; y = cumtrapz(alloy_5(ipt).mfp_cond)/m_si_cond_alloy_ald.extrap(4);
dlmwrite('/home/jason/disorder/si/alloy/accum_c5.dat', [x' y'] ,'delimiter',' ')

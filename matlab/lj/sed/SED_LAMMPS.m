...
%SED Parameters
    ...
    SAMPLE_RATE = 2^4;
    w_step = 2*pi/SAMPLE_RATE; w_max = 2*pi/SEG_LEN;
%Initialize SED object to do averaging over seeds
    str_main=strcat('C:\Documents and Settings\Jason\Desktop\');
    str_read=strcat(str_main,'dump_1_2.vel');
...
vel = zeros(1:NUM_ATOMS,1:SEG_LEN,1:3);
%Store velocity data of all atoms
    for iatom = 1:NUM_ATOMS  
        tic
                vel(iatom,:,1) = dummy{1}(iatom:NUM_ATOMS:length(dummy{1}(:)));
                vel(iatom,:,2) = dummy{2}(iatom:NUM_ATOMS:length(dummy{2}(:)));
                vel(iatom,:,3) = dummy{3}(iatom:NUM_ATOMS:length(dummy{3}(:)));  
        toc
    end

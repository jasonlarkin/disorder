...
%KPTLIST: Load kpt list
    %Windoze
    str_main=strcat('E:\CMU\work\Phonons\LJArgon\Solid\crystal\perfect\SED\4x\');
    str_read=strcat(str_main,'NMD\1atom\GULP\kptlist.dat');
    %Linux
...
%INITIAL POSITIONS: Set initial positions for id matching
    %Windoze
    str_read=strcat(str_main,'NMD\1atom\GULP\x0.data');
    %Linux
...
%SED PARAMETERS: load SED parameters
    %Windoze
    str_read=strcat(str_main,'NMD\1atom\GULP\SED_param.dat');
    %Linux
...
%EIGENVECTORS
        %Windoze
            str_read=strcat(str_main,'NMD\1atom\ME\eigvec.dat');
        %Linux
            %str_read=strcat(str_main,'NMD/1atom/eigvec.dat');
        eigenvec = load(str_read);
%FREQUENCIES
        %Windoze
        str_read=strcat(str_main,'NMD\1atom\ME\freq.dat');
        %Linux
...
%Loop over kpts
        for imode = 1:NUM_MODES
            ...
            for ikpt = 2:NUM_KPTS
                spatial = i*( x0(:,3)*(2*pi./alat(1)).*(SED.kptlist(ikpt,1)) + x0(:,4)*(2*pi./alat(1)).*(SED.kptlist(ikpt,2)) + x0(:,5)*(2*pi./alat(1)).*(SED.kptlist(ikpt,3)) );
                spatial = repmat(spatial,1,NUM_TSTEPS);
%Prepare the eigenvectors to be eigx,y,z(1:NUM_ATOMS,xyz), per mode
                eigx = repmat(conj(eigenvec((3*(ikpt-1)+1),imode)),NUM_UCELL_COPIES,NUM_TSTEPS);
                eigy = repmat(conj(eigenvec((3*(ikpt-1)+2),imode)),NUM_UCELL_COPIES,NUM_TSTEPS);
                eigz = repmat(conj(eigenvec((3*(ikpt-1)+3),imode)),NUM_UCELL_COPIES,NUM_TSTEPS);
...
%Output SED
    %Windoze
        %str_write_total=strcat(str_main,'NMD\1atom\SED_',int2str(iseed),'.txt');
        %str_write_single=strcat(str_main,'NMD\1atom\SED_single_',int2str(iseed),'.txt');
	%Linux
        str_write_total=strcat(str_main,'NMD/1atom/SED_',int2str(iseed),'.txt');
        str_write_single=strcat(str_main,'NMD/1atom/SED_single_',int2str(iseed),'.txt');

...
%EIGENVECTORS
%Windoze
str_read=strcat(str_main,'NMD\eigvec.dat');
%Linux
...
SED.redkpt.sed(1:NUM_OMEGAS,1:NUM_KPTS,1:NUM_MODES) =  0;
    
for ikpt=1:NUM_KPTS  
%Put in group velocities and HLD       
    SED.redkpt.groupvel(1:NUM_MODES,1:3,ikpt) = group_vel((ikpt-1)*NUM_ATOMS_UCELL*3+1:(ikpt)*NUM_ATOMS_UCELL*3,1:3);
    SED.redkpt.freq(ikpt,1:NUM_MODES) = freq(ikpt,1:NUM_MODES);
    tempcnt = 0.0;
    ...
    str_read=strcat(str_main,'NMD\mode\SED_single_',int2str(SED.redkpt.kpt(ikpt,1)),...
    int2str(SED.redkpt.kpt(ikpt,2)),int2str(SED.redkpt.kpt(ikpt,3)),'_',int2str(iseed+2),'.txt');
    dummy = dlmread(str_read,' ');  
        for imode = 1:NUM_MODES
            SED.redkpt.sed(:,ikpt,imode) = SED.redkpt.sed(:,ikpt,imode) + dummy(:,imode+1);
        end
    end
    
SED.redkpt.sed(:,ikpt,:) = SED.redkpt.sed(:,ikpt,:)/NUM_SEED;
...
%Put in HLD  
    SED.irrkpt.HLDfreq(1:NUM_MODES,SED.irrkpt.numirr) = SED.redkpt.freq(ikpt,1:NUM_MODES);
    SED.irrkpt.HLDvel(1:NUM_MODES,1:3,SED.irrkpt.numirr) = SED.redkpt.groupvel(1:NUM_MODES,1:3,ikpt);
    SED.irrkpt.sedavg(:,SED.irrkpt.numirr,1:NUM_MODES) = SED.redkpt.sed(:,ikpt,1:NUM_MODES);
    ...
    SED.irrkpt.sedavg(:,idegen,1:NUM_MODES) = SED.irrkpt.sedavg(:,idegen,1:NUM_MODES) + SED.redkpt.sed(:,ikpt,1:NUM_MODES);
...
%LORENTZIAN FUNCTION FOR SINGLE AND MULTIPLE PEAKS 
            ...
            %weight the three points at the top highest
%             weights(Jmax-4,1)=5;
%             weights(Jmax-3,1)=5;
%             weights(Jmax-2,1)=5;
%             weights(Jmax-1,1)=5;
...
%             weights(Jmax+4,1)=5;
            ...
            options = optimset('MaxIter',50000,'MaxFunEvals',50000,'TolFun',1e-8,'TolX',1e-8);   
            [Ipeak,Jpeak]=max(SED.irrkpt.sedavg(:,kpt_list(kpt_cnt),imode));
            c0 = [ 10*Ipeak, gamma_guess, Jpeak ]; 

...
%KPTLIST: Load kpt list
    str_read=strcat(str_main,'SED\kptlist.dat');
    %Linux
...
%INITIAL POSITIONS: Set initial positions for id matching
    %Windoze
    str_read=strcat(str_main,'SED\x0.data');
    %Linux
...
%SED PARAMETERS: load SED parameters
    %Windoze
    str_read=strcat(str_main,'SED\SED_param.dat');
    %Linux
    ...
    w_plot = NUM_OMEGAS; 
    w_plot_left = 1;
    w_plot_right = NUM_OMEGAS;
    
%FREQUENCIES
        %Windoze
        str_read=strcat(str_main,'SED\freq.dat');
        %Linux
        %str_read=strcat(str_main,'NMD/1atom/freq.dat');
        freq = load(str_read);
%GROUP VEL
        %Windoze
        str_read=strcat(str_main,'SED\vel.dat');
        %Linux
        %str_read=strcat(str_main,'NMD/1atom/freq.dat');
        group_vel = load(str_read);
            
%READ IN FIRST FILE        
        str_read=strcat(str_main,'SED\SED_1.txt');
    ...
    %------AVG OVER SEEDS------------------------------------------------------
        for i1=1:NUM_SEED
                %ME
                str=strcat(str_main,'SED\SED_',int2str(i1),'.txt');
                %ALEX

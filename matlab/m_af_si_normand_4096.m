...
[I(1).I,J(1).J] = find(SED(1).SED.HLDfreq < debye.wcut);
SED(1).SED.life(J(1).J(5:6)) = 2.0*SED(1).SED.life(J(1).J(5:6)); 

subplot(2,1,2),...
loglog(...
    SED(1).SED.HLDfreq(J(1).J),...
    ((1/3))*si.amor.vs_tran^2*(1.0*SED(1).SED.life(J(1).J)*1E-12),'.',...
    Di(8,1).Di(:,2),Di(8,1).Di(:,3),'.',...
...
sio2.debye.cut(2).cond = (con.kb / sio2.VOLUME)*sio2.D(3,3).D(sio2.debye.Icut2,3);
sio2.debye.cut(2).mfp = sqrt(3*sio2.D(3,3).D(sio2.debye.Icut2,3).*sio2.SED(2).SED.life(sio2.debye.Icut2+3)*1E-12 );
debye.cut(2).mfp = 3*Di(8,1).Di(debye.Icut2,3)/si.amor.vs_tran;
...
semilogx(...
    sio2.debye.mfp_film_1000(sio2.debye.Isort) , 1.4 + cumtrapz(sio2.debye.cond_film_1000(sio2.debye.Isort)),...
    regner.sio2.cond(:,1)*1e-6,regner.sio2.cond(:,2)*1.4,'.',...
    [32E-9 45E-9 92E-9 180E-9 190E-9],[1.52 1.34 1.15 1.18 1.31],'.',...
    [45E-9 100E-9 200E-9 500E-9 1000E-9],[0.86 1.22 1.5 1.45 1.42],'.',...
    [1e-11;sio2.debye.mfp_film_1000(sio2.debye.Isort)';1E-5],1.4*[1;ones(length(sio2.debye.mfp_film_1000(sio2.debye.Isort)),1);1]...
    )
axis([1E-9 4E-6 0.0 2.0])
...
semilogx(...
    ...
    galli.sputtered.tf', galli.sputtered.k','.',...
    galli.cvd.tf', galli.cvd.k','.',...
    debye.mfp_film_80000(debye.Isort) , 1.0 +cumtrapz(debye.cond_film_80000(debye.Isort)),...
    debye_w4.mfp_film_80000(debye_w4.Isort) , 0.96 +cumtrapz(debye_w4.cond_film_80000(debye_w4.Isort)),...
    [1e-11;sio2.debye.mfp_film_1000(sio2.debye.Isort)';1E-5],1.0*[1;ones(length(sio2.debye.mfp_film_1000(sio2.debye.Isort)),1);1]...
    )

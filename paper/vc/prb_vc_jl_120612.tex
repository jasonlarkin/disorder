...
modes are defined at the wavevector [000] (referred to herein as Gamma) 
and can couple harmonically to produce a mode thermal diffusivity (Section ).  
...
the VC and of explicitly disordered lattice supercells. 
...
and $a=1.580$ (T=40 K) in LJ units.\cite{mcguaghey_phonon_2004}  
...
exist above approximately $T=20K$.\cite{mcgaughey_phonon_2004}  
...
VC-NMD and VC-ALD require the eigenvalue solution of a Dynamical matrix 
of size $(3n,3n)$ for each irreducible wavevector of the system size 
considered (Section ).(cite) For VC-ALD, the calculation of the phonon 
lifetimes scales as $n^4$,\cite{turney_predicting_2009}  
making calcualtions for large unit cells challenging.(cite) Compared to 
the calculation of 
...
LJ ALD time = 49.2 hours?

LJ VC-NMD time = 

(24 (MD) + <1 (HLD)) hours * 100 cpu = 2400 cpu hour ?

LJ VC-Gamma = MD simulation + eigenvalue solution =  + 3.8 hours
...
It's important to remember that the VC normal modes are exact in the 
limit...

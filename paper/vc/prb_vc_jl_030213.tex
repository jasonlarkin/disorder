...
\begin{figure}
\begin{center}
\mbox{\subfigure{(a)\includegraphics[scale=0.22]
{/home/jason/disorder/si/si_conv_2x2x2_disorder-2.eps}
\subfigure{(b)\includegraphics[scale=0.22]
...
the phonon mode lifetimes are required to predict the thermal 
conductivity using Eq. \eqref{EQ:k_vib}. 
As an alternative to the VC-ALD approach for predicting phonon lifetimes, 
which are discussed in the next section,  
we first use the MD simulation-based normal mode decomposition 
(NMD) method.\cite{ladd_lattice_1986,turney_predicting_2009} The effects 
of disorder are contained in the atomic trajectories (positions and 
velocities) of the MD simulation performed on the explicitly 
disordered supercells (see Section ). 
In NMD, the 
atomic trajectories are first mapped onto the vibrational 
normal mode coordinate and its time derivative,
...
in the $l$th unit cell of the lattice supercell. 
The total energy of a given vibrational mode is calculated from 
...
\end{equation}
where the upper integration limit $t*$ is much larger than the phonon 
lifetime.\cite{larkin_comparison_2012} Equation \eqref{EQ:tau_nmd} 
is derived assuming that the energy correlation follows an 
exponential decay.\cite{turney_predicting_2009} 
...
The MD simulations were 10 times longer than the 
longest phonon lifetime in the system, which can 
be estimated \emph{a priori} from the VC-ALD predicted phonon 
lifetimes. For LJ 
...
Ensemble averaging was performed using ten independent initial 
...
Ref. \citenum{turney_predicting_2009}, 

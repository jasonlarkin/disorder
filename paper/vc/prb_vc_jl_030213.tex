...
Accurate predictions of these vibrational mode properties for 
...
For alloys (i.e., disordered lattices), 
disorder is typically included in the ALD framework 
using Abeles' virtual crystal (VC) 
...
alloy thermal conductivities that compared well with experimental 
measurements. No comprehensive study has been performed 
...
conductivity of the SW silicon alloys are predicted and in 
...
(long-wavelength) limit. In the classical limit, 
...
\end{equation}
where $D_{AF,i}$ is the mode thermal diffusivity and $\omega_i$ is the 
frequency of the $i$th diffuson. The thermal diffusivity of diffusons 
...
Assuming that all vibrational modes scatter over a distance of 
the lattice spacing, 
...
branch-averaged sound speed, $b$ is the number of atoms in the unit 
cell, and $a$ is the lattice constant. The HS limit will be used to 
...
solid is replaced with a perfect, single-species crystal with properties 
such as density and and cohesive energy  
equivalent to an averaging over the disorder 
(e.g., atomic mass and/or bond strength).\cite{abeles_lattice_1963}
The VC approximation is visualized for an alloy in in Figs. 
\ref{F:supercells}(a) and (b), where 
an explicitly mass disordered supercell is replaced by a perfect 
...
\cite{abeles_lattice_1963} With the use of phenomenological  
fitting parameters, good agreement between the predictions and 
experimental measurements 
was found for SiGe and (Ga,In)As alloys. Deviations from the Abeles 
theory were observed for In(As,P) alloys at large concentrations of 
InP, which were attributed to the high mass ratio of 3.7 between 
In and P.\cite{abeles_lattice_1963}

The Abeles theory was developed using the VC approximation 
and Klemens-Callaway theory, which are valid for low-frequency modes and  
...
The application of the Abeles theory leads to good agreement between 
some experimental and computational results. It is important to note that 
...
\cite{cahill_thermal_2004,cahill_thermal_2005} For example, 
as little as $6.2\times10^{19}$ cm$^{-3}$ Ge 
reduces the thermal conductivity of 
Si (mass ratio of 2.6) by a factor of two.\cite{cahill_thermal_2004}  
The relative effects of bond and mass disorder was investigated 
computationally using MD simulations by Skye and 
Schelling for SiGe up to equal concentration,\cite{skye_thermal_2008} 
...
with phonon-phonon and phonon-defect scattering treated as perturbations.
...
Klemens-Callaway theory predictions in the low-frequency, 
low-temperature limit.\cite{} The phonon-defect scattering is treated 
using perturbative methods which can treat mass and/or bond disorder, 
and recover the Klemens theory predictions at low-frequencies. 
...
predictions made by perturbative mass disorder predictions.
\cite{mattis_phonon_1957,tamura_isotope_1983,kamitakahara_vibrations_1974} 
...
isotopically defected GaN, which has relatively small mass ratio of 
the isotopes.\cite{lindsay_thermal_2012} 
...
thermal conductivities at 300 K ($\sim$ 100 W/m-K). 
...
The VC-ALD approach has also been used to predict the phonon lifetimes 
and mean free paths in GaAs/AlAs superlattices, but thermal 
...
An ALD study using phonon properties from DFT calculations for crystalline 
PbTe\cite{shiga_microscopic_2012} 
predicted thermal conductivities ($\sim$ 1 W/m-K) for the perfect systems 
in fair agreement with experiment. For PbTe and PbSe alloys, a VC-ALD 
study predicted a small thermal conductivity reduction with increasing 
concentration.\cite{tian_phonon_2012} Experimental results lack for the 
alloys,\cite{kudman_thermoelectric_1972,pei_convergence_2011} making 
it difficult to asses the validity of the VC-ALD approach for those 
...
limited to VC-ALD 
because of DFT calcualtion costs. Our use of computationally 
...
\begin{figure}
\begin{center}
\mbox{\subfigure{(a)\includegraphics[scale=0.13]
{/home/jason/disorder/si/si_conv_2x2x2_disorder-4.eps}
\subfigure{(b)\includegraphics[scale=0.13]
{/home/jason/disorder/si/si_conv_2x2x2_perfect-4.eps}}}}
\vspace*{0mm}
\end{center}
\caption{\label{F:supercells} 
(a) view of an explicitly disordered supercell of 
...
Concentrations, $c$, of $0.0,0.05,0.15$ and $0.5$ are considered. 
Based on the previous works of others, we only consider mass disorder.

For LJ argon, the lattice constant ($a$)  
...
into agreement with VC-ALD predictions.
\cite{sellan_cross-plane_2010}
...
for vibrational modes of disordered LJ lattice supercells and their 
...
DOS $\propto \omega^2$.\cite{ashcroft_solid_1976} 
Similar agreement between VC and Gamma at low frequencies 
...
ten by a broadening and a shift of the Gamma DOS to higher frequencies 
...
of the dispersion relation of a supercell.
...
\cite{cahill_lattice_1988} However, this is not a good assumption 
...
The structure factor at a virtual crystal wave vector $\kgv$ 
is defined as\cite{allen_diffusons_1999} 
\begin{equation}\label{EQ:SLT}
S^{L,T}\kvcw = 
\sum_{\nu} E^{L,T}\kvcv
\delta (\omega-\omega\kgv),
\end{equation}
where the summation is over the Gamma modes, and $E^{T}$ refers 
...
\right|^2,
\end{equation}
where the summations are over the atoms in the disordered supercell, 
$\pmb{r}_0\ab{l=0}{b}$ refers to the equlibrium atomic positions of the 
mass disordered atoms in the supercell, $\hat{\pmb{\kappa}}_{VC}$ is 
a unit vector.  
Explicit disorder is also included for in the Gamma mode frequencies 
$\omega\kgv$ and eigenvectors $e\kgvba$, which have length $3N_a$. 
...
\footnotemark[1]{For a finite-size system, the 
...
From Fig. \ref{F:SF}, 
an effective dispersion curve (middle panel) can be extracted by 
...
NMD method.\cite{ladd_lattice_1986,turney_predicting_2009-1}  
In NMD, the 
atomic trajectories are first mapped onto the vibrational 
mode coordinate $q\kvt$ and its time derivative $\dot{q}\kvt$,
...
\end{split}
\end{equation}
where $m_b$ is the mass of the $b_{th}$ atom in the unit cell, 
$u_{\alpha}$ is the $\alpha_{th}$ component of the atomic displacement 
from equilibirum and $\dot{u}_{\alpha}$ is the $\alpha_{th}$ 
component of the atomic velocity at time $t$.   
The total energy of each vibrational mode is calculated from 
...
\end{equation}
where the upper integration limit $t^*$ is much larger than the mode  
lifetime, and the brackets indicate an ensemble averaging.
\cite{larkin_comparison_2012} Equation \eqref{EQ:tau_nmd} 
is derived by assuming that the energy auto-correlation follows an 
exponential decay.\cite{turney_predicting_2009-1} 

We perform NMD using the fully disordered supercell and projecting onto 
the frequencies and eigenvectors 
...
longest lifetime in the system, which can 
be estimated \emph{a priori} from the VC-ALD predicted  
lifetimes. For LJ 
...
Ensemble averaging was performed using ten independent initially  
...
group velocities, so that the thermal conductivity can not be predicted 
...
ALD calculations\cite{turney_predicting_2009-1} are based on the 
conventional unit cells of LJ argon and SW silicon with the 
atomic mass based on the concentration.  
...
It has been demonstrated that the effects of 
higher-order phonon processes\cite{ecsedy_thermal_1977} become important at 
temperatures greater than 40 K for LJ argon.\cite{turney_predicting_2009-1} 
The present study is 
concerned with temperatures much less than the melting temperature of 
either LJ argon\cite{mcgaughey_phonon_2004} or 
SW silicon\cite{stillinger_computer_1985} so that we believe ALD will be 
appropriate. We predict the phonon-phonon lifetimes using the method 
...
for mass point defect scattering to second order.\cite{tamura_isotope_1983} 
...
$m^j_{c}$. Tamura used this theory to predict the effect of defect
scattering in isotopic germanium. 
The disordered lifetimes are calcualted using Eqs  
\eqref{EQ:taud_dos} and \eqref{EQ:gn}.\footnotemark[2]{
...
($g_3 \sim 10^{-7}$ and $g_4 \sim 10^{-7}$).
\cite{tamura_isotope_1983} 
For LJ argon and a concentration of 0.15 in this work,  
$g_2 = 0.3018$, $g_3 = -0.3250$ and $g_4 = 0.4411$. 
It is possible that the neglect of the   
higher-order interactions in the Tamura theory 
is responsible for the 
discrepancy of the lifetimes predicted by VC-NMD and Gamma-NMD 
versus VC-ALD at high frequency. Evaluation of the 
higher-order interactions in the Tamura theory is of similar 
complexity to anharmoninc phonon interactions for $n>3$
\cite{ecsedy_thermal_1977,turney_predicting_2009-1} 
...
framework of the VC approximation. 
While studies have been performed on alloying the amorphous phase,
\cite{feldman_thermal_1993} the 
AF theory has not been applied to disordered lattices. In the classical 
harmonic limit, where the specific heat is mode-independent, 
a mode's contribution to the thermal 
conductivity of a perfect crystal is determined by its thermal diffusivity, 
...
\end{equation} The lower limit for phonon thermal diffusivity is 
zero since the group velocities can be zero (e.g., optical modes at the 
Brioullin zone center). Even for large disorder in the alloys, modes 
...
diffusons [Eq. \eqref{EQ:M:k_AF}].
\cite{allen_thermal_1993} In the HS limit,
...
\end{equation}
which leads to    
Eq. \eqref{EQ:M:k_AF,HS}.\footnotemark[3]{
...
For both the VC-NMD and VC-ALD calculations, we approximate  
$\pmb{v}_{g,\mathbf{n}}\kv$ from the VC dispersion 
...
Since the AF theory is harmonic, the thermal diffusivities 
...
\footnotemark[4]{For a finite system, the AF theory 
...
$\delta_{\omega,avg}$.} 
are plotted in Fig. \ref{F:AF} along with the HS limit  
Eq. \eqref{EQ:M:D_HS}, where the sound speed is esimated by 
...
sound speeds calculated from the elastic constants.\cite{gale_general_2003} 
Except at the highest frequencies, the thermal diffusivity of all amorphous 
...
diverge at low frequency.\footnotemark[5]{For the disordered 
lattices studied 
...
VC predicted phonon thermal diffusivity should be $D_{HS}$, which 
is further explored in Sections \ref{S:Thermal Conductivity} and 
\ref{S:SW}. 
Another possible lower-bound of thermal diffusivity based on the 
IR limit is  
\begin{equation}\label{EQ:M:D_IR}
D_{IR} = \frac{2\pi}{3} \frac{v^2_s}{\omega}. 
\end{equation}
The CP model makes the same assumptions for group velocity ($v_s$) and 
lifetime ($2\pi/\omega$).\cite{cahill_lattice_1988} The lower bound 
$D_{IR}$ is considered in Section \ref{S:Thermal Conductivity}.
...
\cite{feldman_numerical_1999,xu_energy_2009} The thermal diffusivity 
...
velocity scale used in Eq. \eqref{EQ:Dph} to calculate 
the thermal diffusivity of high-frequency modes 
in the disordered lattices. 
This statement is supported by considering the 
AF theory predicted mode thermal diffusivities in Fig. \ref{F:AF}, 
which are finite for the LJ alloy at a concentration 
of 0.5. While the thermal diffusivity from Eq. \eqref{EQ:Dph} can be 
zero because of the VC predicted group velocities, this result is not 
consistent with the AF theory predictions. Predictions from model 
...
phonon mode diffusivities will therefore be underpredicted for 
...
\footnotemark[6]{The overprediction of thermal conductivity by VC-ALD 
may be related to the role of disorder in the ALD calculation.
\cite{garg_role_2011,turney_predicting_2009-1} While Garg et al found 
...
\bibliography{/home/jason/Dropbox/ntpl-paper/ntpl-031313}
\end{document}

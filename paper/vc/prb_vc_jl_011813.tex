...
The argon crystal and its thermal-conductivity tensor are
cubically symmetric. The diagonal elements of the thermal-
conductivity tensor are therefore equal so that the thermal
conductivity is a scalar and can be defined by

The SMRT approximation has been shown to be accurate for 

Increased errors are seen for GaN and Diamond, where a full 
iterative solution to the BTE is required for accurate 
results.\cite{ward_intrinsic_2010} 
...
group velocities in disordered systems is just as important as 
...
and polarization, such that $v_g\kv$.  
...
Overall, the lifetimes predicted by both VC-NMD and Gamma-NMD (Fig. ) 
are larger than the Ioffe-Regel limit,\cite{taraskin_determination_1999} 
...
the only known limitation of the Boltzmann ap-
proach is either (a) that each phonon i should have a
sufficiently long mean free path l,. = ~v; ~; that its wave
vector Q can be defined, i.e. , l; »A, , =2m. /~Q~, or alter-
nately (b) if the wave vector Q is impressed externally,
then the resulting oscillation should have a long enough
lifetime (or dephasing time) ~; that its frequency co; is
well defined, i.e., ~;))r;=2m. / ,a.iFor acoustic modes
the two criteria are the same, but for the optic branches
(where group velocity U; is often much smaller than phase
velocity co;/~Q~) the second criterion is less restrictive.
Probably the less restrictive criterion is sufficient; optic
phonons, when they satisfy (b) but not (a), are not con-
tributing much to ~ anyway.
...
from propagating modes which are considered to be phonons. Without 
any detailed mode-by-mode analysi, comparing the thermal 
...
nearly all important modes to thermal transport are non-propagating.
...
should be considered when using the VC approximation, particularly for 
materials with thermal conductivities which are near the 
high-scatter limit.
...
Reciprocal space grids of up to  are required to predict converged 
thermal conductivity of Si.\cite{esfarjani_heat_2011} 
...
converged values of thermal conductivity.\cite{he_lattice_2012} 
We find that similar system sizes are 
also required for 
